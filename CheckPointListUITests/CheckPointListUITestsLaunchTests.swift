...
final class CheckPointListUITestsLaunchTests: XCTestCase {
    ...
    func testOpenAddEventView() throws {
        ...
        XCTAssertTrue(addEventButton.waitForExistence(timeout: 5), "El boton de agregar evento no existe.")
        
        addEventButton.tap()
        
        let eventDetailsSectionHeader: XCUIElement =  app.staticTexts["eventDetailsSectionHeader"]
        
        XCTAssertTrue(eventDetailsSectionHeader.waitForExistence(timeout: 5),
    ...
    func testCreateEvent() throws {
        ...
        XCTAssertTrue(nameTextField.waitForExistence(timeout: 5), "El campo de texto para el nombre no existe.")
        ...
        XCTAssert(eventDatePicker.waitForExistence(timeout: 5), "El date picker para el evento no existe.")
        
        eventDatePicker.tap()

        let dateButton: XCUIElement = eventDatePicker.collectionViews.buttons.element(boundBy: 6)

        XCTAssertTrue(dateButton.waitForExistence(timeout: 5), "El boton del dia 7 no existe.")
        ...
        XCTAssertTrue(saveButton.waitForExistence(timeout: 5), "El boton de guardar no existe.")
        
        saveButton.tap()
        
        let eventItem: XCUIElement = app.staticTexts["event_Event1"]
        
        XCTAssertTrue(eventItem.waitForExistence(timeout: 5),
